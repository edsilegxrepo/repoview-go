...
- **Inferred Metadata**: Distro `ubuntu`, Suite `noble`, Component `main` / `universe`, Arch `amd64`, Format `DEB`.
- **Target Link**: `ubuntu/dists/noble/main/binary-amd64/repoview/index.html`.
...
> [!TIP]
> **Subcommand vs Flag**: Single-repo flags (`--comps`, `--baseurl`, `--ignore-package`, `--exclude-arch`) are irrelevant to a catalog builder. Catalog flags (`--max-depth`, `--exclude`, `--title`, `--output-dir`, `--require-rendered`) are irrelevant to a single-repo build. A dedicated subcommand maintains 100% backward compatibility and clean `--help` outputs.
...
# 5. Bootstrap a starter configuration file from auto-discovery
repoview portal --dump-config portal.yaml /var/www/repos

# 6. Using an explicit configuration file
...
* **Atomic File Generation**: The portal compiles to `index.html.tmp` and performs an atomic filesystem rename, ensuring web servers serving `HOME PATH` never deliver an incomplete 0-byte file during scheduled cron updates.
...
1. **Per-Repo Generator Decoupling**:
   - Add flag to `repoview`:
     ```bash
     repoview --portal-url "../../../../index.html" /var/www/repos/el9/base/x86_64
     ```
   - If `--portal-url` is specified, `repoview` renders a top-bar link: `← All Repositories` pointing to that relative path and saves it into `repoview.json`.
2. **Dynamic Fallback**:
   - If `--portal-url` was not provided at single-repo build time, the per-repo UI uses a dynamic JavaScript fallback:
     - Checks if `document.referrer` originated from the parent portal.
     - If so, displays `← All Repositories` linking back to the referrer or parent directory.
     - Otherwise, gracefully omits the link to prevent broken navigation.
...
| **Unbuilt / Raw Repository** | Broken link if pointing to non-existent repoview | "Raw Mirror" badge linking to directory, or filtered via `--require-rendered` |
...
| **Nested Debian `dists/`** | Missing individual suite/component channels | Specialized Debian crawler descending to `binary-<arch>` leaves |
...
### 7.2 Domain Models & Struct Contracts (`internal/portal/types.go`)
...
// Scan executes directory discovery and returns a sorted Catalog.
...
// Renderer compiles the self-contained portal index.html.
...
// RenderAtomic renders catalog to index.html.tmp, flushes fsync, and renames atomically.
func (r *Renderer) RenderAtomic(catalog *Catalog, outDir string) error
```
...
descriptor := portal.RepoDescriptor{
	...
	PortalURL:       g.config.PortalURL, // Relative link back to parent portal (from --portal-url)
	RepoviewVersion: g.config.Version,
}
if err := writeDescriptor(g.config.OutputDir, descriptor); err != nil {
...
* Add `--portal-url` flag to `cmd/repoview/main.go` and pass into `app.Config`.
...
* Implement filesystem crawler with:
  ...
  * Debian `dists/<suite>/<component>/binary-<arch>` leaf discovery.
  * Inferred taxonomy parsing for Distro, Channel, Arch, and Format.

### Phase 3: Portal Engine & Inline Template
* Build `internal/portal/renderer.go` compiling self-contained `index.html`.
* Embed inline glassmorphism styles, distro SVG symbol bank, and client-side filter engine.
* Implement atomic temporary file writer (`index.html.tmp` $\rightarrow$ rename).

### Phase 4: CLI Subcommand Integration
* Update `cmd/repoview/main.go` to support `repoview portal [options] <home-path>`.
* Support flags: `--title`, `--output-dir`, `--max-depth`, `--require-rendered`, `--dump-config`, `--config`.
...
* Add unit and end-to-end integration tests covering all 4 topologies in `tests/e2e/portal_test.go`.